...
describe("DockerAiContainerLauncher helpers", () => {
  test.concurrent(
    "builds docker args with workspace owner as container user",
    () => {
      const expectedArgs = [
        "--user",
        "501:20",
      ...
      if (os.platform() === "linux") {
        expectedArgs.push("--add-host", "host.docker.internal:host-gateway");
      }

      expectedArgs.push("boboddy/ai-worker:test");
  ...
  test.concurrent(
    "resolves numeric uid and gid from workspace path",
    async () => {
