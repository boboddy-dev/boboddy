/**
 * Integration test for `forceAndVerifyMcpCanary`.
 *
 ...
 *
 * Run with:
 *   BOBODDY_INTEGRATION=true bun test tests/work/step-execution/integration/force-and-verify-mcp-canary.integration.test.ts
 */
...
describe.skipIf(!integrationEnabled)("forceAndVerifyMcpCanary (integration)", () => {
  ...
  test(
    ...
    async () => {
      const logger = createLogger({
        name: "@boboddy/worker",
        level: process.env["BOBODDY_INTEGRATION_VERBOSE"] === "true" ? "debug" : "silent",
      });

      const orchestrator = new DefaultLocalNoWorkspaceRuntimeEnvironmentOrchestrator(logger, {
        payloadProvisioner: new OpencodeRuntimePayloadProvisioner(),
        // No real AI provider credentials are needed — the canary function
        // under test runs against a synthetic fake provider pointed at the
        // fake-LLM server. A SafeProviderAccessResolver lets the host process
        // start even with none resolved.
        ...
        runtimeConfigMaterializer: new SessionRuntimeConfigMaterializer({
          outputBaseDir: path.join(process.cwd(), ".boboddy-tmp-provider-config"),
        }),
      ...
      environment = await orchestrator.launch({
        ...
        fakeAiProviderOverride: { baseUrl: `http://127.0.0.1:${String(fakeAiServer.port)}` },
      });

      const working = await forceAndVerifyMcpCanary({
      ...
      const broken = await forceAndVerifyMcpCanary({
        agentBaseUrl: environment.agentBaseUrl,
        workspaceFolder: environment.workspaceFolder,
        canary: { tool: "fixture_boom", args: {} },
        fakeAiServer,
        timeoutMs: 30_000,
      });
      ...
      if (!broken.passed) {
        ...
        expect(broken.detail).toContain("boom: intentionally broken fixture tool");
      }
    },
    TEST_TIMEOUT_MS,
  );
